...
def receive_embs(pipe: dict) -> dict:
    "Convert embeddings from transformer to node features"
    vocab = list(pipe['unique_products'].keys())
    num_batches = len(vocab)//args.batch_size + int(len(vocab)%args.batch_size!=0)
    embs_lst = []
    with torch.no_grad():
        for batch in tqdm(chunks(vocab, args.batch_size), total=num_batches):
            encoded = tokenizer(batch, padding=True, return_tensors="pt")
            encoded = encoded.to(args.device)
            output = transformer(**encoded, output_hidden_states=False)[1].cpu()
            embs_lst.append(output.detach())
    embs = torch.cat(embs_lst, 0)
    
    pipe['embedding_size'] = embs.shape[1]
    pipe['graph'].x = embs
...
class Net(torch.nn.Module):
    def __init__(self, embedding_size):
        super().__init__()
        self.pre_lin_1 = Linear(embedding_size, 64)
...
def train_pipe(pipe):
    ...
    model = Net(pipe['embedding_size']).to(args.device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--transformer_backbone', type=str, default="bert-base-uncased",
                        help="['bert-base-uncased', 'roberta-base', 'albert-base-v2'...]")
    ...
    transformer = AutoModel.from_pretrained(args.transformer_backbone)
    tokenizer = AutoTokenizer.from_pretrained(args.transformer_backbone)

    transformer = transformer.to(args.device)
    pipe_dict['mask'] = create_mask(pipe_dict['graph'])

    pipe_dict = receive_embs(pipe_dict)
